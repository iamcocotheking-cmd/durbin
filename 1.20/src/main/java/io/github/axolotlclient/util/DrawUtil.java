...
public class DrawUtil {
	...
	private static void fillOutlineQuads(VertexConsumer consumer, Matrix4f matrix, float x1, float x2, float y1, float y2, float z1, float z2, int color) {
		consumer.vertex(matrix, x2, y1, z1).color(color).next();
		consumer.vertex(matrix, x1, y1, z1).color(color).next();
		consumer.vertex(matrix, x1, y2, z1).color(color).next();
		consumer.vertex(matrix, x2, y2, z1).color(color).next();

		consumer.vertex(matrix, x1, y1, z2).color(color).next();
		consumer.vertex(matrix, x2, y1, z2).color(color).next();
		consumer.vertex(matrix, x2, y2, z2).color(color).next();
		consumer.vertex(matrix, x1, y2, z2).color(color).next();
