...
public class CreditsScreen extends Screen {
	...
	@Override
	public void init() {
		addDrawable(new TextWidget(getTitle(), textRenderer));
		addDrawableChild(new CreditsList(client, width, height, height - 33 - 33, 33, 25));

		var back = addDrawableChild(ButtonWidget.builder(CommonTexts.BACK, buttonWidget -> closeScreen())
			.position(width / 2 - 75, height - 33 / 2 - 20 / 2).build());

		addDrawableChild(ButtonWidget.builder(Text.translatable("creditsBGM").append(": ")
			...
			for (String t : credit.things) {
				...
				if (t.startsWith("http")) {
					addDrawableChild(new PlainTextButtonWidget(width / 2 - textWidth / 2, startY, textWidth, 12,
						(Text) Text.literal(t).br$color(ClientColors.SELECTOR_GREEN), btn ->
					...
					addDrawable(new TextWidget(width / 2 - textWidth / 2, startY, textWidth, 12,
						(Text) Text.literal(t).br$color(ClientColors.SELECTOR_GREEN), textRenderer));
				}
				startY += 12;
			}
