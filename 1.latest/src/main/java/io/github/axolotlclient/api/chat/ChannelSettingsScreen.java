...
public class ChannelSettingsScreen extends Screen {
	...
	protected ChannelSettingsScreen(Screen parent, Channel channel) {
		super(Component.translatable("api.channel.configure", channel.getName()));
		this.parent = parent;
	...
	protected void init() {
		...
		LinearLayout names = content.addChild(LinearLayout.vertical().spacing(20 - minecraft.font.lineHeight + 8 - 5));
		...
		countDisabler.set(sliderAssembly(options::addChild, val -> (int) (val * 100d), count::set, channel.getPersistence().count() / 100d));
		names.addChild(text("api.chat.groups.persistence.duration", "api.chat.groups.persistence.duration.tooltip"));
		durationDisabler.set(sliderAssembly(options::addChild, val -> (long) (val * 100d), duration::set, channel.getPersistence().duration() / 100d));
		countDisabler.get().accept(false);
		durationDisabler.get().accept(false);

		var namesInput = options.addChild(new EditBox(minecraft.font, 150, 20, Component.empty()));
		names.addChild(text("api.chat.groups.add_participants"));
	...
	private <T> Consumer<Boolean> sliderAssembly(Consumer<Layout> layoutConsumer, Function<Double, T> valueFunc, Consumer<Supplier<T>> value, double initialValue) {

		AtomicReference<T> currentVal = new AtomicReference<>();
		var slider = new AbstractSliderButton(0, 0, 128, 20, Component.empty(), initialValue) {
			@Override
		...
		var text = new EditBox(minecraft.font, 128, 20, Component.empty());
	...
	private LayoutElement text(String translationKey) {
		return new StringWidget(150, font.lineHeight, Component.translatable(translationKey), font);
	}

	private LayoutElement text(String translationKey, String tooltipKey) {
		var widget = new StringWidget(150, font.lineHeight, Component.translatable(translationKey), font);
