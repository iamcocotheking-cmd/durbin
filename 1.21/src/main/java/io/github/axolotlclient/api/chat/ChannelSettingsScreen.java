...
public class ChannelSettingsScreen extends Screen {
	...
	protected void init() {
		...
		names.add(text("api.chat.groups.add_participants"));
	...
	private <T> Consumer<Boolean> sliderAssembly(Consumer<Widget> layoutConsumer, Function<Double, T> valueFunc, Consumer<Supplier<T>> value, double initialValue) {

		AtomicReference<T> currentVal = new AtomicReference<>();
		var slider = new SliderWidget(0, 0, 128, 20, Text.empty(), initialValue) {
			@Override
		...
		}, true).sprite(Identifier.of(AxolotlClientCommon.MODID, "cursor"), 8, 8).dimensions(20, 20).build();
