...
public class CropImageScreen extends Screen {
	...
	private final HeaderFooterLayoutWidget hfl = new HeaderFooterLayoutWidget(this, 33, 80);
	...
	@Override
	protected void init() {
		hfl.addToHeader(getTitle(), textRenderer);
		imageWidget = hfl.addToContents(new ImageWidget(hfl.getX(), hfl.getHeaderHeight(), hfl.getWidth(), hfl.getContentsHeight()));
		var footer = hfl.addToFooter(LinearLayoutWidget.createVertical()).setSpacing(4);
		var inputs = footer.add(LinearLayoutWidget.createHorizontal(), LayoutSettings::alignHorizontallyCenter).setSpacing(4);
		inputs.add(new TextWidget(Text.translatable("gallery.image.crop.position"), textRenderer)).setHeight(20);
		posX = inputs.add(new TextFieldWidget(textRenderer, 50, 20, Text.translatable("gallery.image.crop.inputs.x")));
		posY = inputs.add(new TextFieldWidget(textRenderer, 50, 20, Text.translatable("gallery.image.crop.inputs.y")));
		inputs.add(new TextWidget(Text.translatable("gallery.image.crop.size"), textRenderer)).setHeight(20);
		posW = inputs.add(new TextFieldWidget(textRenderer, 50, 20, Text.translatable("gallery.image.crop.inputs.width")));
		posH = inputs.add(new TextFieldWidget(textRenderer, 50, 20, Text.translatable("gallery.image.crop.inputs.height")));
		...
		var footerLine2 = footer.add(LinearLayoutWidget.createHorizontal(), LayoutSettings::alignHorizontallyCenter).setSpacing(4);
		var footerLine3 = footer.add(LinearLayoutWidget.createHorizontal(), LayoutSettings::alignHorizontallyCenter).setSpacing(4);
		footerLine2.add(ButtonWidget.builder(Text.translatable("gallery.image.crop.save_as"), btn -> {
			String result = null;
		...
		footerLine2.add(ButtonWidget.builder(Text.translatable("gallery.image.crop.save"), btn -> {
		...
		footerLine2.add(ButtonWidget.builder(Text.translatable("gallery.image.crop.copy"), btn -> {
			try (var crop = imageWidget.getCopyOfSelection()) {
		...
		footerLine3.add(ButtonWidget.builder(CommonTexts.BACK, btn -> closeScreen()).build());
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
		...
		guiGraphics.drawTexture(Screen.HEADER_SEPARATOR, 0, hfl.getHeaderHeight() - 2, 0.0F, 0.0F, this.width, 2, 32, 2);
		guiGraphics.drawTexture(Screen.FOOTER_SEPARATOR, 0, this.height - this.hfl.getFooterHeight(), 0.0F, 0.0F, this.width, 2, 32, 2);
	...
	protected void repositionElements() {
		if (imageWidget != null) {
			imageWidget.setHeight(hfl.getContentsHeight());
		...
		protected void drawWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
			...
			if (isMouseOver(mouseX, mouseY)) {
				if (hasControlDown()) {
					...
					((CursorType) currentHandle.cursor).select();
				} else {
					hoveredHandle = getHandle(getTransformedX(mouseX), getTransformedY(mouseY));
					if (hoveredHandle != null) {
						((CursorType) hoveredHandle.cursor).select();
					} else {
						CursorTypes.ARROW.select();
					}
				}
