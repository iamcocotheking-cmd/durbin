...
public class SessionStatsHudEntryConfigScreen extends Screen {
	...
	protected void init() {
		...
		layout.addToContents(new EntryList(client, layout.getWidth(), layout.getContentsHeight(), layout.getHeaderHeight(), 24));
		layout.addToFooter(ButtonWidget.builder(CommonTexts.BACK, btn -> closeScreen()).build());
	...
	private class EntryList extends ElementListWidget<ListEntry> {
		...
		public EntryList(MinecraftClient minecraft, int width, int height, int y, int itemHeight) {
