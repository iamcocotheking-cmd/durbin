...
public class AxolotlClientToast extends DrawUtil implements Toast {
	...
	public Visibility draw(GuiGraphics graphics, ToastManager manager, long startTime) {
		...
		if (this.messageLines.isEmpty()) {
			...
			graphics.drawText(MinecraftClient.getInstance().textRenderer, title, textOffset, 7, -256, false);

			for (int i = 0; i < this.messageLines.size(); i++) {
				graphics.drawText(MinecraftClient.getInstance().textRenderer, this.messageLines.get(i), textOffset, 18 + i * LINE_SPACING, -1, false);
			}
		}

		double d = (double) DISPLAY_TIME_MILLIS * manager.getNotificationDisplayTime();
		return (double) startTime < d ? Visibility.SHOW : Visibility.HIDE;
	}
}
