...
public class RenderUtil {
	...
	public void fill(Matrix4f matrix, int x1, int y1, int x2, int y2, int color,
		...
		bufferBuilder.xyz(matrix, x1, y2, 0.0f).color(r, g, b, a);
		bufferBuilder.xyz(matrix, x2, y2, 0.0f).color(r, g, b, a);
		bufferBuilder.xyz(matrix, x2, y1, 0.0f).color(r, g, b, a);
		bufferBuilder.xyz(matrix, x1, y1, 0.0f).color(r, g, b, a);
	...
	public void fillBlend(GuiGraphics graphics, int x, int y, int width, int height, Color color) {
		fillBlend(graphics.getMatrices().peek().getModel(), x, y, x + width, y + height, color.toInt());
	}
	...
	public void fillBlend(Matrix4f matrix, int x1, int y1, int x2, int y2, int color) {
		...
		BufferBuilder bufferBuilder = Tessellator.getInstance().begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
		bufferBuilder.xyz(matrix, (float) x1, (float) y2, 0.0F).color(red, green, blue, alpha);
		bufferBuilder.xyz(matrix, (float) x2, (float) y2, 0.0F).color(red, green, blue, alpha);
		bufferBuilder.xyz(matrix, (float) x2, (float) y1, 0.0F).color(red, green, blue, alpha);
		bufferBuilder.xyz(matrix, (float) x1, (float) y1, 0.0F).color(red, green, blue, alpha);
		BufferRenderer.drawWithShader(bufferBuilder.endOrThrow());
	}
}
